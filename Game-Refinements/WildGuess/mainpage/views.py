...
def log_score(request):
    if request.method == 'POST':
        ...
        score_value = int(data.get('score'))

        # Initialize session dict if not present
        if 'game_scores' not in request.session:
            request.session['game_scores'] = {}

        # Update or insert score for that animal
        request.session['game_scores'][animal] = score_value
...
def final_score_page(request):
    game_scores = request.session.get('game_scores', {})
    total_score = sum(game_scores.values())

    # Only save if not already saved in this session
    if not request.session.get('scores_saved', False):
        for animal, score_value in game_scores.items():
            score.objects.create(animal=animal, scoreVal=score_value, totalScore=total_score)
        request.session['scores_saved'] = True  # Mark as saved
